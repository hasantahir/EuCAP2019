...
\section{Methodology}
%
Here we simulate a full-scale model of a magnesium cardiovascular stent and monitor the corrosion using the  was shown in \cite{bowen2015rates} that the corrosion rate $P$, for a wire like structure is independent of length and can be completely described in terms of the radius $r(t)$ of the wire at any given time $t$ by,
%
\begin{equation}
  P = \frac{r_0 - r(t)}{t}
  \label{eq:corrosion}
\end{equation}
%
where $r_0$ is the initial wire radius. For a magnesium wire, an in-vivo penetration corrosion rate of \SI{500}{\um \per \year} would
To simulate the scattering properties of an absorbable cardiovascular stent, we used the \emph{Open stents} CAD models that are customizable due to their inherent parameterization \cite{bonsignore_open_2012}. The stent length was set to \SI{25}{\mm} with the diameter in the expanded form equal to \SI{4}{\mm}, as shown in Fig. \ref{fig:stent}. The stent strut thickness was initially set to \SI{170}{\um}.
\begin{figure}[h!]
\centering
\includegraphics[width=2.5in]{Stent.PNG}
